#!/usr/bin/env python3
""" Create Test for  GithubOrgClient class in the client module"""
...
class TestGithubOrgClient(unittest.TestCase):
    ...
    def test_public_repos(self, mock_repo):
        ...
        with patch.object(GithubOrgClient,
            ...
            client_test = GithubOrgClient('holberton')
            repo_test = client_test.public_repos()
            for idx in range(3):
                self.assertIn(mock_repo.return_value[idx]['name'], repo_test)
    ...
    def test_has_license(self, repo, license_key, expected):
        ...
        """
        instance_test = GithubOrgClient('holberton')
        available_license = instance_test.has_license(repo, key_license)
        self.assertEqual(available_license, expected)
